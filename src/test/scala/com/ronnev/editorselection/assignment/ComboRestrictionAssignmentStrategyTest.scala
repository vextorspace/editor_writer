...
class ComboRestrictionAssignmentStrategyTest extends FeatureSpec {
    feature("ComboRestrictionAssignmentStrategy creates group assignments that do not violate restrictions") {
        ...
        scenario("An empty history restriction, no other restrictions, 5 by 4 with 3 per") {
            ...
            assert(result.editorsPerWriterA().map(_._2.size).filterNot(_ == 3).isEmpty)
            assert(result.editorsPerWriterB().map(_._2.size).filterNot(_ == 3).isEmpty)
            assert(result.writersPerEditorA().map(_._2.size).filterNot(s => {s == 3 || s == 4}).isEmpty)
            assert(result.writersPerEditorB().map(_._2.size).filterNot(s => {s == 2 || s == 3}).isEmpty)
        }
        ...
        scenario("A single history restriction, no other restrictions") {
            ...
            assert(result.editorsPerWriterA()("barney").diff(List("wilma", "betty", "kate")).size >= 2)
            assert(result.editorsPerWriterA()("ted").diff(List("wilma", "bob", "kate")).size >= 2)
            assert(result.editorsPerWriterA()("ned").diff(List("betty", "bob", "kate")).size >= 2)
            assert(result.editorsPerWriterB()("wilma").diff(List("fred", "barney", "ted")).size >= 2)
            assert(result.editorsPerWriterB()("betty").diff(List("fred", "barney", "ned")).size >= 2)
            assert(result.editorsPerWriterB()("bob").diff(List("fred", "ted", "ned")).size >= 2)
            assert(result.editorsPerWriterB()("kate").diff(List("barney", "ted", "ned")).size >= 2)
        }
    ...
    feature("processing one writer should assign a valid combo given the rules") {
        scenario("A history restriction 2 lessers") {
            ...
            val editorsFunc = () => Map("wilma" -> List("fred", "bob"), "betty" -> List("fred", "bob"), "bob" -> List("fred"), "kate" -> List("amy"))
            ...
            val newGroup = assignment.copy()
            ComboRestrictionAssignmentStrategy.doOneWriter(writer, editorCombos, editorsFunc, newGroup, List(restriction))

            assert(newGroup.editorsPerWriterA()(writer).sorted == List("wilma", "bob").sorted)
        }

        scenario("A history restriction 1 lesser") {

        }

        scenario("A bad pair restriction 2 lessers") {

        }

        scenario("A bad pair restriction 1 lesser") {

        }

        scenario("A history restriction and a bad pair restriction 2 lessers") {

        }

        scenario("A history restriction and a bad pair restriction 1 lesser") {
